...
########################################################
############# FROM NEW DASHBOARD VERIFICATIONS
########################################################
...
def new_dashboard_email_new_verification_code(request, email, user):
    old_verification = request.new_email_proofing_state_db.get_state_by_eppn_and_email(
            user.eppn, email, raise_on_missing=False)
    if old_verification is not None:
        log.debug('removing old verification code:'
                                 ' {!r}.'.format(old_verification.to_dict()))
        request.new_email_proofing_state_db.remove_state(old_verification)
    ...
    request.new_email_proofing_state_db.save(verification_state)

    log.info('Created new email verification code '
                            'for user {!r} and email {!r}.'.format(user, email))
    log.debug('Verification Code:'
                             ' {!r}.'.format(verification_state.to_dict()))

    return code


def new_dashboard_verify_mail_address(request, state, proofing_user):
    """
    ...
    log.info('Email address {!r} confirmed '
                            'for user {!r}'.format(state.verification.email, proofing_user))
    request.new_email_proofing_state_db.remove_state(state)
    log.debug('Removed proofing state: {} '.format(state))


def new_dashboard_phone_new_verification_code(request, phone, user):
    old_verification = request.new_phone_proofing_state_db.get_state_by_eppn_and_mobile(
                       user.eppn, phone, raise_on_missing=False)
    if old_verification is not None:
        log.debug('removing old verification code:'
                                 ' {!r}.'.format(old_verification.to_dict()))
        request.new_phone_proofing_state_db.remove_state(old_verification)
    ...
    # XXX This should be an atomic transaction together with saving
    # the user and sending the letter.
    request.new_phone_proofing_state_db.save(verification_state)
    log.info('Created new mobile verification code '
