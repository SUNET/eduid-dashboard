...
from eduiddashboard.verifications import get_verification_code, verify_code
...
def letter_proof_user():
    ...
    if rdata.get('verified', False):
        ...
        if env['request'].idproofinglog.log_verification(proofing_data):
            ...
            code_data = get_verification_code(env['request'], 'norEduPersonNIN', obj_id=rdata['number'], user=user)
            try:
                # This is a hack to reuse the existing proofing functionality, the users code is
                # verified by the micro service
                verify_code(env['request'], 'norEduPersonNIN', code_data['code'])
